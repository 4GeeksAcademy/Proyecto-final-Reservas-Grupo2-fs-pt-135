...
business_category = db.Table(
    "business_category",
    db.Column("business_profile_id", db.Integer, db.ForeignKey(
        "business_profile.id"), primary_key=True),
...
class Category(db.Model):
    ...
    businesses: Mapped[list["BusinessProfile"]] = relationship(
        "BusinessProfile", secondary=business_category, back_populates="categories")
...
class ClientProfile(db.Model):
    ...
    favorite_businesses: Mapped[list["FavoriteBusiness"]] = relationship(
        "FavoriteBusiness", back_populates="client", cascade="all, delete-orphan")
...
class BusinessProfile(db.Model):
    ...
    services: Mapped[list["Service"]] = relationship(
        "Service", back_populates="business")
    portfolio: Mapped["BusinessPortfolio"] = relationship(
        "BusinessPortfolio", back_populates="business_profile", uselist=False)
    gallery_images: Mapped[list["BusinessGallery"]] = relationship(
        "BusinessGallery", back_populates="business_profile")
    working_schedule: Mapped["BusinessWorkingSchedule"] = relationship(
        "BusinessWorkingSchedule", back_populates="business", uselist=False)
...
class BusinessPortfolio(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    business_profile_id: Mapped[int] = mapped_column(
        ForeignKey("business_profile.id"), unique=True, nullable=False)
    logo_url: Mapped[str] = mapped_column(String(255), nullable=True)
    description: Mapped[str] = mapped_column(String(1000), nullable=True)
    business_profile: Mapped["BusinessProfile"] = relationship(
        "BusinessProfile", back_populates="portfolio")
...
class BusinessGallery(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    business_profile_id: Mapped[int] = mapped_column(
        ForeignKey("business_profile.id"), nullable=False)
    image_url: Mapped[str] = mapped_column(String(255), nullable=False)
    business_profile: Mapped["BusinessProfile"] = relationship(
        "BusinessProfile", back_populates="gallery_images")
...
class Reservas(db.Model):
    ...
    client_id: Mapped[int] = mapped_column(
        ForeignKey("client_profile.id"), nullable=False)
    ...
    status: Mapped[str] = mapped_column(
        String(30), nullable=False, default="Activa")
    ...
    def serialize(self):
        business = self.service.business if self.service else None

        business_logo = None
        business_image = None

        if business and business.portfolio:
            business_logo = business.portfolio.logo_url

        if business and business.gallery_images:
            business_image = business.gallery_images[0].image_url
...
class FavoriteBusiness(db.Model):
    ...
    client_id = db.Column(db.Integer, db.ForeignKey(
        "client_profile.id"), nullable=False)
    business_id = db.Column(db.Integer, db.ForeignKey(
        "business_profile.id"), nullable=False)

    client = db.relationship(
        "ClientProfile", back_populates="favorite_businesses")
    business = db.relationship("BusinessProfile")

    __table_args__ = (
        db.UniqueConstraint("client_id", "business_id",
                            name="unique_client_business_favorite"),
    )
